...
vinyl-dj-library/
  ...
  api/
    main.py            -- FastAPI app, mounts routers + static files, renders index page.
                          Computes "last synced" as max(Release.discogs_synced_at)
                          across the collection (was hardcoded to None — real bug,
                          status bar always said "Not synced yet" even after a real sync).
    routes/
      ...
      tap_bpm.py            -- GET /tap-bpm (redirected to /analyze), accepts optional
                              track_id; fetches preset track and passes title/artist/
                              existing BPM+key to the template
      analyze.py            -- POST /analyze-clip (WAV upload → librosa → {bpm, key,
                              camelot, confidence}); GET /track-search?q= (JSON search
                              sorted title > artist > release, includes existing_bpm/
                              existing_key for overwrite confirmation)
  sync/
  ...
  enrich/
    ...
    audio_analysis.py  -- fully implemented: librosa BPM (beat_track, start_bpm=120)
                          + key (Krumhansl-Schmuckler chroma correlation over 24
                          pitch classes). Returns Match(bpm, key, source=
                          "local_analysis", confidence). Requires optional 'audio'
                          extra (pip install librosa).
  web/
...
- Status bar correctly shows "Last synced: 2026-06-21 23:20 UTC" (the real timestamp from the real sync) instead of the stale hardcoded "Not synced yet".
- `GET /tap-bpm`: simulated precisely-timed taps (500ms apart, JS `setTimeout`) and got 119.6 BPM against an exact target of 120 — within normal timing jitter, confirming the rolling-average math is correct. Verified the reset button clears state, and that a 2.5s gap between taps correctly starts a fresh count rather than computing a bogus ~24 BPM from the long gap. Verified the spacebar shortcut also registers taps.
- Static assets (`htmx.min.js`, `htmx-json-enc.js`, `tap-bpm.js`, `style.css`) serve correctly.
...
- *(2026-06-25)* Built local audio analysis and redesigned the analyze page. Probed Deezer as a BPM data source (85% artist match rate, 0% BPM fill — Deezer's BPM field is not populated in their API; dead end). Decided on local librosa analysis as the practical path for the remaining ~68% unmatched tracks. Implemented `enrich/audio_analysis.py`: `librosa.beat.beat_track(start_bpm=120)` for BPM (start_bpm biases toward common DJ tempos, reducing half/double-time misdetection), Krumhansl-Schmuckler chroma correlation for key (24 pitch classes, ~70% accuracy on tonal music). Added `POST /analyze-clip` (WAV upload → tempfile → librosa → {bpm, key, camelot, confidence}) and `GET /track-search?q=` (JSON track search sorted by title > artist > release priority, includes existing_bpm/existing_key for the overwrite confirmation flow) in `api/routes/analyze.py`. Chose Web Audio API `ScriptProcessorNode` → in-browser WAV encoding instead of MediaRecorder to avoid an ffmpeg dependency (MediaRecorder outputs WebM/MP4 which librosa can't read without ffmpeg; raw PCM → 44-byte RIFF header → Blob('audio/wav') needs no server-side dep). BPM result is an editable `<input type="number">` flanked by ÷2/×2 buttons so the user can correct the common half/double-time error (confirmed on a real Overmono track: librosa returned 86 instead of 130). Added colour-coded source provenance badges on every BPM/key cell (getsongbpm=blue, local_analysis=purple, manual=amber, beatport=orange) and overwrite confirmation (hx-confirm on the htmx edit form, window.confirm() in JS). Renamed the nav link from "Tap BPM" to "Analyze". Added "Analyze this track" link on the featured-track banner and made tracklist rows clickable links to /track/{id}. Extended `PATCH /track/{id}/bpm-key` to accept an optional `source` field (defaults "manual"). Redesigned the /tap-bpm page into a unified analysis page: "Now analyzing: [Track] — [Artist]" header when arriving via ?track_id=X; Record Clip and Tap Tempo panels side by side in a flex grid; shared "Save to track" section at the bottom with editable BPM + Key fields (auto-populated from whichever tool runs), a direct Save button for the preset track, and a search for any other track. "Use this BPM →" button on the tap panel (appears after ≥2 taps) feeds the tapped BPM into the shared save section with source="manual". Source set to local_analysis for clip results, manual for tap/typed values. Verified end-to-end in preview: both the track-filtered (?track_id=1) and unfiltered views, Use-this-BPM button, and full layout.
...
- *(2026-06-22)* User triage of remaining open items: fix the stale "Not synced yet" status (done — see below); negative-result caching explicitly deferred pending a decision on adding another BPM/key data source for the 237 still-unresolved tracks; BPM-range/Camelot-key filtering explicitly deprioritized for now. Fixed `api/main.py`'s `index()`: `last_synced` was hardcoded to `None`, so the status bar always said "Not synced yet" even with a fully synced real collection — now computed as `max(Release.discogs_synced_at)`, confirmed live showing the real timestamp. Built a new live tap-tempo BPM page (`GET /tap-bpm`, `web/templates/tap_bpm.html`, `web/static/tap-bpm.js`), reachable via a "Tap BPM" link added to `base.html` (top-left, every page) — the fastest manual way to get a usable BPM for a record with no data anywhere, while standing at the decks: tap or press space in time with the beat, rolling average of the last 8 taps, auto-resets after a 2s gap so an interruption doesn't corrupt the running average. Verified live with simulated precisely-timed taps (500ms apart → 119.6 BPM vs. an exact target of 120) and the gap-reset behavior (2.5s gap correctly discarded the prior tap instead of computing a bogus ~24 BPM). The page also has a "Currently playing" section stubbed "coming soon", reserved for a planned future mic-based live-analysis feature meant to sit above the tap counter on the same page. 7 net new/changed tests (163 total).
